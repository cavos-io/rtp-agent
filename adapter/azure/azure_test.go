...
func TestAzureSTTStreamRejectsReferenceChannelCountChange(t *testing.T) {
	provider, err := NewAzureSTT("key", "eastus")
	if err != nil {
		t.Fatalf("NewAzureSTT error = %v", err)
	}
	...
	if err := stream.PushFrame(&model.AudioFrame{
		...
		t.Fatalf("PushFrame first channel count error = %v", err)
	}
	if err := stream.PushFrame(&model.AudioFrame{
...
func TestAzureSTTStreamTurnEndReturnsAPIConnectionError(t *testing.T) {
	requests := make(chan *http.Request, 1)
	configMessages := make(chan string, 1)
	...
	case <-time.After(time.Second):
		_ = stream.Close()
		<-serverClosed
		t.Fatal("Next timed out after Azure turn.end, want APIConnectionError")
	}
}
...
func TestAzureSTTProviderCloseClosesActiveStreams(t *testing.T) {
	requests := make(chan *http.Request, 1)
	configMessages := make(chan string, 1)
	serverClosed := make(chan struct{}, 1)
...
func TestAzureSTTClosedStreamNextReturnsEOF(t *testing.T) {
	requests := make(chan *http.Request, 1)
	configMessages := make(chan string, 1)
	serverClosed := make(chan struct{})
...
func TestAzureSTTStreamAfterCloseIsRejected(t *testing.T) {
	...
	provider.dialWebsocket = func(context.Context, string, http.Header) (*websocket.Conn, *http.Response, error) {
		dials++
		return nil, nil, errors.New("unexpected azure stt dial")
	}
...
func TestAzureSTTUpdateOptionsReconnectsActiveStreamBeforeNextAudio(t *testing.T) {
	requests := make(chan *http.Request, 2)
	configMessages := make(chan string, 2)
	...
	secondReq := receiveAzureTestValue(t, requests, "second request after update")
	if got := secondReq.URL.Query().Get("language"); got != "id-ID" {
		t.Fatalf("second stream language = %q, want updated language id-ID", got)
	}
	secondConfig := receiveAzureTestValue(t, configMessages, "second speech config after update")
	_, configBody := splitAzureTestMessage(t, []byte(secondConfig))
	var configPayload struct {
		Properties map[string]string `json:"properties"`
	}
...
func azureTestWaitForSessionStarted(t *testing.T, stream stt.RecognizeStream) {
	t.Helper()
	concrete, ok := stream.(*azureSTTStream)
	if !ok {
		t.Fatalf("stream = %T, want *azureSTTStream", stream)
	...
	for {
		concrete.mu.Lock()
		started := concrete.sessionStarted
		concrete.mu.Unlock()
		if started {
			return
		}
		select {
		case <-deadline:
			t.Fatal("timed out waiting for Azure STT session start")
		default:
			time.Sleep(time.Millisecond)
		}
	}
...
func azureTestHoldOpenDialer(
	...
	return func(ctx context.Context, endpoint string, headers http.Header) (*websocket.Conn, *http.Response, error) {
		clientConn, serverConn := net.Pipe()
		errCh := make(chan error, 1)
		go runAzureTestHoldOpenWebsocketServer(serverConn, requests, configMessages, serverClosed, errCh)
		...
		go func() {
			if serverErr := <-errCh; serverErr != nil && !isAzureTestWebsocketCleanupError(serverErr) {
				t.Errorf("test hold-open websocket server: %v", serverErr)
			}
		}()
		return conn, resp, nil
	}
...
func azureTestTurnEndHoldOpenDialer(
	t *testing.T,
	requests chan<- *http.Request,
	configMessages chan<- string,
	serverClosed chan<- struct{},
	...
	return func(ctx context.Context, endpoint string, headers http.Header) (*websocket.Conn, *http.Response, error) {
		clientConn, serverConn := net.Pipe()
		errCh := make(chan error, 1)
		go runAzureTestTurnEndHoldOpenWebsocketServer(serverConn, requests, configMessages, serverClosed, errCh)
		...
		go func() {
			if serverErr := <-errCh; serverErr != nil && !isAzureTestWebsocketCleanupError(serverErr) {
				t.Errorf("test turn-end websocket server: %v", serverErr)
			}
		}()
		return conn, resp, nil
	}
...
func azureTestSessionStartedHoldOpenDialer(
	t *testing.T,
	requests chan<- *http.Request,
	configMessages chan<- string,
	serverClosed chan<- struct{},
	...
	return func(ctx context.Context, endpoint string, headers http.Header) (*websocket.Conn, *http.Response, error) {
		clientConn, serverConn := net.Pipe()
		errCh := make(chan error, 1)
		go runAzureTestSessionStartedHoldOpenWebsocketServer(serverConn, requests, configMessages, serverClosed, errCh)
		...
		go func() {
			if serverErr := <-errCh; serverErr != nil && !isAzureTestWebsocketCleanupError(serverErr) {
				t.Errorf("test session-started hold-open websocket server: %v", serverErr)
			}
		}()
		return conn, resp, nil
	}
...
func runAzureTestSessionStartedHoldOpenWebsocketServer(
	conn net.Conn,
	requests chan<- *http.Request,
	configMessages chan<- string,
	serverClosed chan<- struct{},
...
func runAzureTestTurnEndHoldOpenWebsocketServer(
	conn net.Conn,
	requests chan<- *http.Request,
	configMessages chan<- string,
	serverClosed chan<- struct{},
	...
	if err := writeAzureTestWebsocketFrame(conn, websocket.TextMessage, []byte("Path: turn.end\r\nContent-Type: application/json\r\n\r\n{}")); err != nil {
		errCh <- err
		return
	}
	for {
