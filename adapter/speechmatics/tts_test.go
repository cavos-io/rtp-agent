...
func TestSpeechmaticsTTSStreamAfterCloseIsRejected(t *testing.T) {
	provider := NewSpeechmaticsTTS("test-key")
	if err := tts.Close(provider); err != nil {
		t.Fatalf("provider Close error = %v", err)
	}
...
func TestSpeechmaticsTTSPreservesReferenceNegativeSampleRate(t *testing.T) {
	provider := NewSpeechmaticsTTS("test-key", WithSpeechmaticsTTSSampleRate(-1))

	if provider.sampleRate != -1 {
		t.Fatalf("sample rate = %d, want explicit reference sample rate -1", provider.sampleRate)
	}
	if got := provider.SampleRate(); got != -1 {
		t.Fatalf("SampleRate = %d, want explicit reference sample rate -1", got)
	}
	...
	assertSpeechmaticsTTSQuery(t, req.URL.Query(), "output_format", "pcm_-1")
}
...
func TestSpeechmaticsTTSSynthesizePostsAndStreamsPCM(t *testing.T) {
	...
	if !audio.IsFinal {
		t.Fatal("audio IsFinal = false, want reference final frame at EOF")
	}
	...
	if next, err := stream.Next(); next != nil || err != io.EOF {
		t.Fatalf("second Next = (%+v, %v), want EOF after final audio", next, err)
	}
}

func TestSpeechmaticsTTSChunkedStreamMarksReferenceFinalAudioAtEOF(t *testing.T) {
	...
	audio, err := stream.Next()
	if err != nil {
		t.Fatalf("Next error = %v, want final audio frame", err)
	}
	if audio == nil || audio.Frame == nil {
		t.Fatalf("Next audio = %+v, want final audio frame", audio)
	}
	if !audio.IsFinal {
		t.Fatalf("audio IsFinal = false, want reference final frame at EOF")
	}
	if !bytes.Equal(audio.Frame.Data, []byte{0x01, 0x02}) {
		t.Fatalf("audio data = %v, want provider PCM bytes", audio.Frame.Data)
	}
	if next, err := stream.Next(); next != nil || err != io.EOF {
		t.Fatalf("second Next = (%+v, %v), want EOF after final audio", next, err)
	}
...
func TestSpeechmaticsTTSSynthesizeUsesReferenceShortRequestID(t *testing.T) {
	...
	if !audio.IsFinal {
		t.Fatal("audio IsFinal = false, want reference final audio")
	}
	if next, err := stream.Next(); next != nil || err != io.EOF {
		t.Fatalf("second Next = (%+v, %v), want EOF after final audio", next, err)
	}
...
func TestSpeechmaticsTTSSynthesizeRetriesReferenceTransientErrorBeforeAudio(t *testing.T) {
	...
	audio, err := stream.Next()
	if err != nil {
		t.Fatalf("Next() error = %v, want retry to recover", err)
	}
	...
	if requests != 2 {
		t.Fatalf("requests = %d, want one retry before audio", requests)
	}
}
...
func TestSpeechmaticsTTSSynthesizeRetriesReferenceNoAudioBeforeAudio(t *testing.T) {
	...
	audio, err := stream.Next()
	if err != nil {
		t.Fatalf("Next() error = %v, want retry after no-audio response", err)
	}
	if audio == nil || audio.Frame == nil || string(audio.Frame.Data) != string([]byte{0x03, 0x04}) {
		t.Fatalf("Next() audio = %+v, want PCM from retried request", audio)
	}
	if requests != 2 {
		t.Fatalf("requests = %d, want one retry after no audio", requests)
	}
}
...
func TestSpeechmaticsTTSChunkedStreamBuffersPartialSamples(t *testing.T) {
	...
	if !audio.IsFinal {
		t.Fatalf("first Next = %+v, want final audio with trailing partial byte discarded", audio)
	}
...
func TestSpeechmaticsTTSChunkedStreamUsesReferenceProgressivePCMFrames(t *testing.T) {
	...
	for i, wantSamples := range []uint32{160, 640, 1280} {
		audio, err := stream.Next()
		if err != nil {
	...
	if final == nil || !final.IsFinal || final.Frame == nil {
		t.Fatalf("final Next = %+v, want final tail audio", final)
	}
	if final.Frame.SamplesPerChannel != 160 {
		t.Fatalf("final samples = %d, want reference 10ms tail", final.Frame.SamplesPerChannel)
	}
}
...
func TestSpeechmaticsTTSChunkedStreamUsesReferenceFrameSizeForNonKilohertzRate(t *testing.T) {
	...
	for i, wantSamples := range []uint32{441, 1764, 3528, 7056, 8800} {
		audio, err := stream.Next()
		if err != nil {
			t.Fatalf("Next frame %d error = %v", i, err)
		}
	...
	if final.Frame.SamplesPerChannel != 441 {
		t.Fatalf("final samples = %d, want reference 10ms tail", final.Frame.SamplesPerChannel)
	}
}

func TestSpeechmaticsTTSChunkedStreamEmitsReferenceFinalAudio(t *testing.T) {
	...
	if audio == nil || audio.Frame == nil || !audio.IsFinal {
		t.Fatalf("first Next = %+v, want reference final audio frame", audio)
	}
	if got, want := body.closeCount, 1; got != want {
