...
func TestSpeechmaticsTTSSynthesizeRequestUsesReferenceOptions(t *testing.T) {
	...
	assertSpeechmaticsTTSQuery(t, query, "sm-sdk", "livekit-plugins-v0.1.7")
...
func TestSpeechmaticsTTSSynthesizePostsAndStreamsPCM(t *testing.T) {
	...
	if audio.IsFinal {
		t.Fatal("audio IsFinal = true, want flushed audio before reference final marker")
	}
...
func TestSpeechmaticsTTSChunkedStreamFlushesReferenceAudioBeforeFinalMarkerAtEOF(t *testing.T) {
	...
	if audio == nil || audio.Frame == nil {
		t.Fatalf("Next audio = %+v, want flushed audio frame", audio)
	}
	if audio.IsFinal {
		t.Fatalf("audio IsFinal = true, want flushed audio before reference final marker")
	}
...
func TestSpeechmaticsTTSSynthesizeRetriesReferenceStatusErrorBeforeAudio(t *testing.T) {
	withSpeechmaticsTTSRetryInterval(t, 0)
	originalClient := http.DefaultClient
	requests := 0
	http.DefaultClient = &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
	...
	audio, err := stream.Next()
	if err != nil {
		t.Fatalf("Next() error = %v, want retry after status error before audio", err)
	}
	if audio == nil || audio.Frame == nil || string(audio.Frame.Data) != string([]byte{0x01, 0x02}) {
		t.Fatalf("Next() audio = %+v, want PCM from retried request", audio)
	}
	if requests != 2 {
		t.Fatalf("requests = %d, want one retry after status error before audio", requests)
	}
}
...
func TestSpeechmaticsTTSChunkedStreamBuffersPartialSamples(t *testing.T) {
	...
	if audio.IsFinal {
		t.Fatalf("first Next = %+v, want flushed audio with trailing partial byte discarded before final marker", audio)
	}
	final, err := stream.Next()
	if err != nil {
...
func TestSpeechmaticsTTSChunkedStreamUsesReferenceProgressivePCMFrames(t *testing.T) {
	...
	for i, wantSamples := range []uint32{160, 640, 1280, 160} {
		audio, err := stream.Next()
		if err != nil {
...
func TestSpeechmaticsTTSChunkedStreamUsesReferenceFrameSizeForNonKilohertzRate(t *testing.T) {
	...
	for i, wantSamples := range []uint32{441, 1764, 3528, 7056, 8800, 441} {
		audio, err := stream.Next()
		if err != nil {
			t.Fatalf("Next frame %d error = %v", i, err)
		}
...
func TestSpeechmaticsTTSChunkedStreamEmitsReferenceFinalMarker(t *testing.T) {
	...
	if audio == nil || audio.Frame == nil || audio.IsFinal {
		t.Fatalf("first Next = %+v, want flushed audio before reference final marker", audio)
	}
	final, err := stream.Next()
	if err != nil {
