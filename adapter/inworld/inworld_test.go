...
func TestInworldTTS_Synthesize(t *testing.T) {
	...
	tts := NewInworldTTS("apiKey", "voice", WithTTSURL(server.URL), WithTTSHttpClient(server.Client()))
...
func TestInworldLLM_Initialization(t *testing.T) {
	l := NewInworldLLM("apiKey", "model")
	if l == nil {
		t.Fatal("NewInworldLLM returned nil")
	}
}
