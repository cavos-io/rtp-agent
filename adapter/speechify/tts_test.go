...
func TestSpeechifyTTSChunkedStreamEmitsReferenceFinalMarker(t *testing.T) {
	stream := &speechifyTTSChunkedStream{
		resp:       &http.Response{Body: io.NopCloser(bytes.NewReader(speechifyTestWAV([]byte{0x01, 0x00}, 24000, 1)))},
		sampleRate: 24000,
	}
	defer stream.Close()
