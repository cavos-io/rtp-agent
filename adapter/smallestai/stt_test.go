...
func TestSmallestAISTTStreamAfterCloseIsRejected(t *testing.T) {
	...
	provider := NewSmallestAISTT("test-key",
		withSmallestAISTTWebsocketDialer(func(context.Context, string, http.Header) (*websocket.Conn, *http.Response, error) {
			dials++
			return nil, nil, errors.New("unexpected smallestai stt dial")
		}),
	)
	if err := provider.Close(); err != nil {
		t.Fatalf("Close: %v", err)
...
func TestSmallestAISTTNextReturnsQueuedTranscriptBeforeStreamError(t *testing.T) {
	for range 64 {
		stream := &smallestAISTTStream{
			events: make(chan *stt.SpeechEvent, 1),
			errCh:  make(chan error, 1),
...
func TestSmallestAISTTRecognizeReturnsAPIStatusError(t *testing.T) {
	oldClient := http.DefaultClient
	http.DefaultClient = &http.Client{Transport: smallestAIRoundTripFunc(func(r *http.Request) (*http.Response, error) {
		return &http.Response{
