...
func TestPluginDownloadFilesUsesResourcesModels(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)

	var gotURL string
	var gotPath string
	oldDownload := downloadModelFile
	downloadModelFile = func(url, path string) error {
		gotURL = url
		gotPath = path
		return os.WriteFile(path, []byte("model"), 0o600)
	}
	t.Cleanup(func() { downloadModelFile = oldDownload })
	...
	wantPath := filepath.Join(dir, "resources", "models", "ten-vad.onnx")
	if gotPath != wantPath {
		t.Fatalf("download path = %q, want %q", gotPath, wantPath)
	}
	if !strings.Contains(gotURL, "githubusercontent.com/TEN-framework/ten-vad/") {
		t.Fatalf("download URL = %q, want direct TEN upstream URL", gotURL)
	}
...
func TestPluginDownloadFilesSkipsExistingModel(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)
