...
func TestCartesiaTTSConstructorOptionsMatchReference(t *testing.T) {
	...
	msg := buildCartesiaOptions(provider, true)
	if msg["language"] != "es" {
...
func TestCartesiaStreamOptionsUseReferenceDefaults(t *testing.T) {
	provider := NewCartesiaTTS("test-key", "", "")
...
func TestCartesiaTTSStreamTextPacketsIncludeReferenceOptions(t *testing.T) {
	...
	provider := NewCartesiaTTS("test-key", "voice-1", "sonic-3",
		WithCartesiaBaseURL("http://cartesia.test"),
		WithCartesiaAudioFormat("pcm_s16le", 16000),
		WithCartesiaLanguage("fr"),
		WithCartesiaWordTimestamps(false),
	)
	stream, err := provider.Stream(context.Background())
	...
	if err := stream.PushText("Bonjour tout le monde. Tail"); err != nil {
		t.Fatalf("PushText error = %v", err)
	}
	...
	if !ok || voice["mode"] != "id" || voice["id"] != "voice-1" {
		t.Fatalf("voice = %#v, want reference voice id packet", msg["voice"])
	}
	outputFormat, ok := msg["output_format"].(map[string]any)
	if !ok || outputFormat["encoding"] != "pcm_s16le" || outputFormat["sample_rate"] != float64(16000) {
		t.Fatalf("output_format = %#v, want reference output format", msg["output_format"])
	}
	...
	case <-closed:
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for capture websocket close")
	}
	select {
	case err := <-serverErr:
		if err != nil {
			t.Fatalf("capture websocket server error: %v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for capture websocket server")
	}
}
...
func TestCartesiaTTSStreamMalformedMessageReturnsAPIConnectionError(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	serverErr := make(chan error, 1)
	go runCartesiaReadThenMalformedWebsocketServer(serverConn, serverErr)
	...
	case <-time.After(time.Second):
		t.Fatal("Next timed out, want malformed provider message to terminate stream like reference")
	}
	...
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for malformed message server")
	}
}

func TestCartesiaTTSStreamMalformedAudioReturnsAPIConnectionError(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	serverErr := make(chan error, 1)
	go runCartesiaReadThenMalformedAudioWebsocketServer(serverConn, serverErr)
	...
	case <-time.After(time.Second):
		t.Fatal("Next timed out, want malformed provider audio to terminate stream like reference")
	}
	...
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for malformed audio server")
	}
}
...
func TestCartesiaTTSStreamAudioDoneAfterEndInputEmitsFinalMarker(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for audio done server")
	}
}
...
func TestCartesiaTTSStreamEmitsWordTimestamps(t *testing.T) {
	...
	if got := audio.TimedTranscript[0]; got.Text != "hello " || got.StartTime != 0.1 || got.EndTime != 0.2 {
		t.Fatalf("TimedTranscript[0] = %#v, want hello with fallback space 0.1-0.2", got)
	}
	if got := audio.TimedTranscript[1]; got.Text != "world " || got.StartTime != 0.3 || got.EndTime != 0.5 {
		t.Fatalf("TimedTranscript[1] = %#v, want world with fallback space 0.3-0.5", got)
	}
...
func TestCartesiaTTSStreamAlignsWordTimestampSpacing(t *testing.T) {
	...
	if got := audio.TimedTranscript[0]; got.Text != "hello" {
		t.Fatalf("TimedTranscript[0].Text = %q, want hello", got.Text)
	}
...
func runCartesiaCaptureMessageWebsocketServer(conn net.Conn, messages chan<- map[string]any, closed chan<- struct{}, errCh chan<- error) {
	upgrader := websocket.Upgrader{}
	listener := &singleCartesiaConnListener{conn: conn}
	server := &http.Server{
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			_, _, err = ws.ReadMessage()
			if err == nil {
				errCh <- errors.New("expected websocket close after captured message")
				return
			}
			close(closed)
			errCh <- nil
...
func runCartesiaReadThenMalformedWebsocketServer(conn net.Conn, errCh chan<- error) {
	upgrader := websocket.Upgrader{}
	listener := &singleCartesiaConnListener{conn: conn}
	server := &http.Server{
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if err := ws.WriteMessage(websocket.TextMessage, []byte(`{"type":"chunk"`)); err != nil {
				errCh <- err
				return
			}
			_, _, err = ws.ReadMessage()
			if err == nil {
				errCh <- errors.New("expected client close after malformed message")
				return
			}
			errCh <- nil
		}),
...
func runCartesiaReadThenMalformedAudioWebsocketServer(conn net.Conn, errCh chan<- error) {
	upgrader := websocket.Upgrader{}
	listener := &singleCartesiaConnListener{conn: conn}
	server := &http.Server{
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			_, _, err = ws.ReadMessage()
			if err == nil {
				errCh <- errors.New("expected client close after malformed audio")
				return
			}
			errCh <- nil
		}),
