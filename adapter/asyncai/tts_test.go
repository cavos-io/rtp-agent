...
func TestAsyncAITTSStreamDialFailureReturnsAPIConnectionError(t *testing.T) {
	oldDialer := websocket.DefaultDialer
	websocket.DefaultDialer = &websocket.Dialer{
		NetDialContext: func(context.Context, string, string) (net.Conn, error) {
			return nil, errors.New("asyncai websocket dial failed")
		},
		Proxy: nil,
	}
...
func TestAsyncAITTSAudioFromWebsocketMessage(t *testing.T) {
	...
	audio, done, err = asyncAITTSAudioFromWebsocketMessage([]byte(`{"context_id":"ctx-1","error":"provider diagnostic"}`), 32000)
	if err != nil || audio != nil || done {
		t.Fatalf("error diagnostic = audio=%+v done=%v err=%v, want ignored websocket message", audio, done, err)
	}
}
...
func TestAsyncAITTSWebsocketLogMessageDoesNotAbortReferenceStream(t *testing.T) {
	upgrader := websocket.Upgrader{}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		conn, err := upgrader.Upgrade(w, r, nil)
		if err != nil {
		...
		defer conn.Close()
		messages := [][]byte{
			[]byte(`{"context_id":"ctx-1","error":"provider diagnostic"}`),
			[]byte(`{"context_id":"ctx-1","final":true}`),
		}
	...
	final, err := stream.Next()
	if err != nil {
		t.Fatalf("Next final error = %v, want final marker after provider log message", err)
	}
	if final == nil || !final.IsFinal || final.SegmentID != "ctx-1" {
		t.Fatalf("final = %#v, want reference final marker with segment", final)
	}
...
func TestAsyncAITTSWebsocketInvalidAudioDoesNotAbortReferenceStream(t *testing.T) {
	upgrader := websocket.Upgrader{}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		conn, err := upgrader.Upgrade(w, r, nil)
		if err != nil {
	...
	final, err := stream.Next()
	if err != nil {
		t.Fatalf("Next final error = %v, want final marker after invalid audio", err)
	}
	if final == nil || !final.IsFinal || final.SegmentID != "ctx-1" {
		t.Fatalf("final = %#v, want reference final marker with segment", final)
	}
