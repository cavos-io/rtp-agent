...
func TestAsyncAITTS_Synthesize(t *testing.T) {
	...
	tts := NewAsyncAITTS("apiKey", "voice", WithTTSURL(server.URL), WithTTSHttpClient(server.Client()))
...
func TestAsyncAILLM_Initialization(t *testing.T) {
	l := NewAsyncAILLM("apiKey", "model")
	if l == nil {
		t.Fatal("NewAsyncAILLM returned nil")
	}
}
