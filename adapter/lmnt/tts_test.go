...
func TestLMNTTTSSynthesizeDefersReferenceRequestUntilNext(t *testing.T) {
...
func TestLMNTTTSChunkedStreamEmitsReferenceFinalMarkerAfterEmptyMP3Audio(t *testing.T) {
	stream := &lmntTTSChunkedStream{
...
func TestLMNTTTSChunkedStreamEmitsReferenceFinalMarkerAfterRawAudio(t *testing.T) {
	stream := &lmntTTSChunkedStream{
		resp:       &http.Response{Body: io.NopCloser(bytes.NewReader([]byte{0x01, 0x02}))},
		format:     "raw",
		sampleRate: 16000,
...
func TestLMNTTTSChunkedStreamNextAfterCloseReturnsEOF(t *testing.T) {
	stream := &lmntTTSChunkedStream{
		resp:       &http.Response{Body: io.NopCloser(strings.NewReader("audio"))},
		format:     "raw",
		sampleRate: 24000,
	}
...
type lmntCloseCountBody struct {
	*strings.Reader
	closeCount int
}

func (b *lmntCloseCountBody) Close() error {
	b.closeCount++
	if b.closeCount > 1 {
