...
func TestGradiumSTT_Recognize(t *testing.T) {
	...
	s := NewGradiumSTT("apiKey", WithSTTURL(server.URL), WithSTTHttpClient(server.Client()))
...
func TestGradiumTTS_Synthesize(t *testing.T) {
	server := testutils.NewMockServer(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	...
	tts := NewGradiumTTS("apiKey", "voice", WithTTSURL(server.URL), WithTTSHttpClient(server.Client()))
...
func TestGradiumLLM_Initialization(t *testing.T) {
	l := NewGradiumLLM("apiKey", "model")
	if l == nil {
		t.Fatal("NewGradiumLLM returned nil")
	}
}
