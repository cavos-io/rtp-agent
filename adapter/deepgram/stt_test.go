...
func TestDeepgramSTTRecognizeMalformedChannelReturnsAPIConnectionError(t *testing.T) {
	oldClient := http.DefaultClient
	http.DefaultClient = &http.Client{Transport: deepgramRoundTripFunc(func(r *http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}},
			Body:       io.NopCloser(strings.NewReader(`{"metadata":{"request_id":"req-bad-channel"},"results":{"channels":[{}]}}`)),
			Request:    r,
		}, nil
	})}
	t.Cleanup(func() { http.DefaultClient = oldClient })
...
func TestDeepgramSTTRecognizeMalformedNullAlternativesReturnsAPIConnectionError(t *testing.T) {
	oldClient := http.DefaultClient
	http.DefaultClient = &http.Client{Transport: deepgramRoundTripFunc(func(r *http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}},
			Body:       io.NopCloser(strings.NewReader(`{"metadata":{"request_id":"req-null-alts"},"results":{"channels":[{"alternatives":null}]}}`)),
			Request:    r,
		}, nil
	})}
	t.Cleanup(func() { http.DefaultClient = oldClient })
...
func TestDeepgramSTTRecognizeMalformedNullWordsReturnsAPIConnectionError(t *testing.T) {
	oldClient := http.DefaultClient
	http.DefaultClient = &http.Client{Transport: deepgramRoundTripFunc(func(r *http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}},
			Body:       io.NopCloser(strings.NewReader(`{"metadata":{"request_id":"req-null-words"},"results":{"channels":[{"alternatives":[{"transcript":"bad","confidence":0.5,"words":null}]}]}}`)),
			Request:    r,
		}, nil
	})}
	t.Cleanup(func() { http.DefaultClient = oldClient })
...
func TestDeepgramSTTStreamIgnoresReferenceBinaryMessages(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	serverErr := make(chan error, 1)
	go runDeepgramSTTBinaryThenValidWebsocketServer(serverConn, serverErr)
	...
	start := nextDeepgramTestSpeechEvent(t, stream)
	if start.Type != stt.SpeechEventStartOfSpeech {
		t.Fatalf("first event type = %s, want start_of_speech from valid text Results", start.Type)
	}
	final := nextDeepgramTestSpeechEvent(t, stream)
	if final.Type != stt.SpeechEventFinalTranscript || len(final.Alternatives) != 1 || final.Alternatives[0].Text != "valid" {
		t.Fatalf("second event = %+v, want valid final transcript after ignored binary message", final)
	}
	end := nextDeepgramTestSpeechEvent(t, stream)
	if end.Type != stt.SpeechEventEndOfSpeech {
		t.Fatalf("third event type = %s, want end_of_speech from valid text Results", end.Type)
	}
	if err := stream.Close(); err != nil {
		t.Fatalf("Close() error = %v", err)
	}
...
func TestDeepgramSTTUpdateOptionsMatchesReferenceFutureRequests(t *testing.T) {
	...
	streamURL, err = url.Parse(buildDeepgramStreamURL(provider, "en-US"))
	if err != nil {
		t.Fatalf("parse stream url after empty base URL update: %v", err)
	}
	if streamURL.Scheme != "" || streamURL.Host != "" {
		t.Fatalf("stream URL after empty base URL update = %q, want empty reference endpoint", streamURL.String())
	}
}
...
func TestDeepgramSTTProviderCloseCancelsPendingRecognize(t *testing.T) {
	requests := make(chan *http.Request, 1)
	oldClient := http.DefaultClient
	http.DefaultClient = &http.Client{Transport: deepgramRoundTripFunc(func(r *http.Request) (*http.Response, error) {
		requests <- r
	...
	errCh := make(chan error, 1)
	go func() {
		event, err := provider.Recognize(ctx, []*model.AudioFrame{{
			Data:              []byte{0x01, 0x02},
			SampleRate:        16000,
			NumChannels:       1,
			SamplesPerChannel: 1,
		...
		if event != nil {
			errCh <- fmt.Errorf("Recognize returned event after provider Close: %+v", event)
			return
		}
		errCh <- err
	}()
...
func TestDeepgramSTTStreamDropsReferenceMalformedResultMissingAlternatives(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	serverErr := make(chan error, 1)
	closeServer := make(chan struct{})
	go runDeepgramMalformedChannelThenValidWebsocketServer(serverConn, closeServer, serverErr)
	...
	final := nextDeepgramTestSpeechEvent(t, stream)
	if final.Type != stt.SpeechEventFinalTranscript || len(final.Alternatives) != 1 || final.Alternatives[0].Text != "valid" {
		t.Fatalf("second event = %+v, want valid final transcript with no false end_of_speech first", final)
	}
	end := nextDeepgramTestSpeechEvent(t, stream)
	if end.Type != stt.SpeechEventEndOfSpeech {
		t.Fatalf("third event type = %s, want end_of_speech", end.Type)
...
func TestDeepgramSTTStreamDropsReferenceMalformedResultNullAlternatives(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	serverErr := make(chan error, 1)
	closeServer := make(chan struct{})
	go runDeepgramNullAlternativesThenValidWebsocketServer(serverConn, closeServer, serverErr)
...
func TestDeepgramSTTStreamDropsReferenceMalformedAlternativeNullWords(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	serverErr := make(chan error, 1)
	closeServer := make(chan struct{})
	go runDeepgramNullWordsThenValidWebsocketServer(serverConn, closeServer, serverErr)
...
func TestDeepgramSTTStreamDropsReferenceMalformedMultiLanguageEndpoint(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	serverErr := make(chan error, 1)
	closeServer := make(chan struct{})
	go runDeepgramMalformedMultiLanguageThenValidWebsocketServer(serverConn, closeServer, serverErr)
...
func runDeepgramSTTBinaryThenValidWebsocketServer(conn net.Conn, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
	...
	go func() {
		for {
			if _, _, err := readDeepgramSTTTestClientWebsocketFrame(reader); err != nil {
				return
			}
		}
	}()
	if err := writeDeepgramTestWebsocketFrame(conn, websocket.BinaryMessage, []byte(`{"type":"Results","is_final":true,"speech_final":true,"metadata":{"request_id":"req-binary"},"channel":{"alternatives":[{"transcript":"binary","confidence":0.9,"words":[]}]}}`)); err != nil {
		errCh <- err
		return
	}
	if err := writeDeepgramTestWebsocketFrame(conn, websocket.TextMessage, []byte(`{"type":"Results","is_final":true,"speech_final":true,"metadata":{"request_id":"req-valid"},"channel":{"alternatives":[{"transcript":"valid","confidence":0.9,"words":[]}]}}`)); err != nil {
...
func runDeepgramMalformedChannelThenValidWebsocketServer(conn net.Conn, closeServer <-chan struct{}, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
...
func runDeepgramNullAlternativesThenValidWebsocketServer(conn net.Conn, closeServer <-chan struct{}, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
	...
	for _, message := range []string{
		`{"type":"SpeechStarted"}`,
		`{"type":"Results","is_final":false,"speech_final":true,"metadata":{"request_id":"req-bad"},"channel":{"alternatives":null}}`,
		`{"type":"Results","is_final":true,"speech_final":true,"metadata":{"request_id":"req-valid"},"channel":{"alternatives":[{"transcript":"valid","confidence":0.9,"words":[]}]}}`,
	} {
		if err := writeDeepgramTestWebsocketFrame(conn, websocket.TextMessage, []byte(message)); err != nil {
			errCh <- err
...
func runDeepgramNullWordsThenValidWebsocketServer(conn net.Conn, closeServer <-chan struct{}, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
	...
	for _, message := range []string{
		`{"type":"SpeechStarted"}`,
		`{"type":"Results","is_final":false,"speech_final":true,"metadata":{"request_id":"req-bad"},"channel":{"alternatives":[{"transcript":"bad","confidence":0.5,"words":null}]}}`,
		`{"type":"Results","is_final":true,"speech_final":true,"metadata":{"request_id":"req-valid"},"channel":{"alternatives":[{"transcript":"valid","confidence":0.9,"words":[]}]}}`,
	} {
		if err := writeDeepgramTestWebsocketFrame(conn, websocket.TextMessage, []byte(message)); err != nil {
			errCh <- err
...
func runDeepgramMalformedMultiLanguageThenValidWebsocketServer(conn net.Conn, closeServer <-chan struct{}, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
	...
	for _, message := range []string{
		`{"type":"SpeechStarted"}`,
		`{"type":"Results","is_final":false,"speech_final":true,"metadata":{"request_id":"req-bad"},"channel":{"alternatives":[{"transcript":"bad","confidence":0.5,"languages":[],"words":[{"word":"bad","start":0.1,"end":0.4}]}]}}`,
		`{"type":"Results","is_final":true,"speech_final":true,"metadata":{"request_id":"req-valid"},"channel":{"alternatives":[{"transcript":"valid","confidence":0.9,"languages":["es"],"words":[]}]}}`,
	} {
		if err := writeDeepgramTestWebsocketFrame(conn, websocket.TextMessage, []byte(message)); err != nil {
			errCh <- err
			return
