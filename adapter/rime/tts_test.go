...
func TestRimeTTSExpiredPooledWebsocketCloseIsDeferredLikeReference(t *testing.T) {
	var dials atomic.Int32
	firstEOS := make(chan struct{})
	releaseFirstClose := make(chan struct{})
	var releaseFirstCloseOnce sync.Once
	releaseFirst := func() {
		releaseFirstCloseOnce.Do(func() { close(releaseFirstClose) })
	}
	...
	websocket.DefaultDialer = &websocket.Dialer{
		NetDialContext: func(context.Context, string, string) (net.Conn, error) {
			...
			switch dials.Add(1) {
			case 1:
				go rimeTestServeReusableWebsocketBlockOnEOS(t, server, firstEOS, releaseFirstClose)
			case 2:
				close(secondDialStarted)
				go func() {
					defer server.Close()
					if err := rimeTestWebsocketHandshake(server); err != nil {
						t.Errorf("second websocket handshake: %v", err)
						return
					}
	...
	case <-firstEOS:
		t.Fatal("expired pooled websocket close started before fresh dial; want deferred reference close")
	case <-time.After(150 * time.Millisecond):
		t.Fatal("fresh websocket dial did not start before expired close blocked")
	}
	releaseFirst()
	select {
	case result := <-resultCh:
		if result.err != nil {
			t.Fatalf("second Stream error = %v", result.err)
		}
		if result.stream == nil {
			t.Fatal("second Stream = nil, want fresh stream")
		}
		_ = result.stream.Close()
	case <-time.After(time.Second):
		t.Fatal("second Stream did not finish after deferred close release")
	}
	<-secondAccepted
	_ = provider.Close()
}
...
func rimeTestServeReusableWebsocketBlockOnEOS(t *testing.T, conn net.Conn, eosSeen chan<- struct{}, release <-chan struct{}) {
	t.Helper()
	defer conn.Close()
	if err := rimeTestWebsocketHandshake(conn); err != nil {
		t.Errorf("websocket handshake: %v", err)
...
func TestRimeTTSAudioFromWebsocketMessage(t *testing.T) {
	...
	zeroFloatStarts, done, transcript, err := rimeTTSAudioFromWebsocketMessage([]byte(`{"type":"timestamps","word_timestamps":{"words":["hi"],"start":0.0,"end":[0.2]}}`), 24000)
	if err != nil {
		t.Fatalf("zero-float timestamp starts message: %v", err)
	}
	if zeroFloatStarts != nil || done || transcript != "" {
		t.Fatalf("zero-float timestamp starts = audio:%+v done:%v transcript:%q, want ignored like reference", zeroFloatStarts, done, transcript)
	}
	...
	if _, _, _, err := rimeTTSAudioFromWebsocketMessage([]byte(`{"type":"error","message":["a\u0001b"]}`), 24000); err == nil {
		t.Fatal("control string array error message returned nil error, want stream error")
	} else {
		var apiErr *llm.APIError
		if !errors.As(err, &apiErr) {
			t.Fatalf("control string array error message error = %T %v, want APIError", err, err)
		}
		if apiErr.Message != `Rime ws error: ['a\x01b']` {
			t.Fatalf("control string array APIError message = %q, want Python-style escaped string repr", apiErr.Message)
		}
	}
}
...
func TestRimeTTSAudioFromWebsocketMalformedPayloadReturnsAPIConnectionError(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "non-ascii audio",
			payload: []byte(`{"type":"chunk","data":"AQ\u263aIDBA=="}`),
		},
		{
