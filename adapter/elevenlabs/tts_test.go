...
func TestElevenLabsTTSStreamNextReturnsAPIConnectionErrorOnDialFailure(t *testing.T) {
	oldDialer := websocket.DefaultDialer
	websocket.DefaultDialer = &websocket.Dialer{
		NetDialContext: func(context.Context, string, string) (net.Conn, error) {
			return nil, errors.New("dial refused")
...
func TestElevenLabsStreamURLForwardsReferenceLanguageForConfiguredModel(t *testing.T) {
	...
	regional, err := NewElevenLabsTTS("test-key", "voice-1", "eleven_multilingual_v2",
		WithElevenLabsLanguage("zh-CN"),
	)
	if err != nil {
		t.Fatalf("NewElevenLabsTTS() regional error = %v", err)
	}
	parsedRegional, err := url.Parse(buildElevenLabsStreamURL(regional))
	if err != nil {
		t.Fatalf("parse regional stream url: %v", err)
	}
...
func TestElevenLabsTTSStreamsShareReferenceWebsocketConnection(t *testing.T) {
	serverErr := make(chan error, 1)
	dialCount := make(chan struct{}, 2)
	clientConn, serverConn := net.Pipe()
	go runElevenLabsTTSSharedContextWebsocketServer(serverConn, serverErr)
	...
	audioA, err := streamA.Next()
	if err != nil {
		t.Fatalf("Next A error = %v", err)
	}
	if got := audioA.Frame.Data; !bytes.Equal(got, []byte{1, 0, 2, 0}) {
		t.Fatalf("A audio = %v, want first context audio", got)
	}
	finalA, err := streamA.Next()
	if err != nil {
		t.Fatalf("Next A final error = %v", err)
	}
	if finalA == nil || !finalA.IsFinal {
		t.Fatalf("A final = %#v, want final marker", finalA)
	}
	if next, err := streamA.Next(); next != nil || err != io.EOF {
		t.Fatalf("A third Next = (%#v, %v), want EOF", next, err)
	}
	...
	if next, err := streamB.Next(); next != nil || err != io.EOF {
		t.Fatalf("B third Next = (%#v, %v), want EOF", next, err)
	}
...
func TestElevenLabsTTSPendingSharedDialSurvivesFirstStreamCloseLikeReference(t *testing.T) {
	serverErr := make(chan error, 1)
	...
	dialCount := make(chan struct{}, 2)
	clientConn, serverConn := net.Pipe()
	go runElevenLabsTTSOneContextWebsocketServerNoDrain(serverConn, []byte{3, 0, 4, 0}, serverErr)
	...
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for pending websocket dial")
	}
	streamB, err := provider.Stream(context.Background())
	if err != nil {
		t.Fatalf("Stream B error = %v", err)
	...
	release()
	if err := streamB.PushText("bravo charlie"); err != nil {
		t.Fatalf("PushText B error = %v, want sibling stream to survive first stream close during pending dial", err)
	}
	if err := streamB.Flush(); err != nil {
		t.Fatalf("Flush B error = %v", err)
	}
	...
	if got := audioB.Frame.Data; !bytes.Equal(got, []byte{3, 0, 4, 0}) {
		t.Fatalf("B audio = %v, want second stream audio", got)
	}
	finalB, err := streamB.Next()
	if err != nil {
		t.Fatalf("Next B final error = %v", err)
...
func TestElevenLabsTTSUpdateOptionsInvalidatesReferenceWebsocketConnection(t *testing.T) {
	...
	if got := firstAudio.Frame.Data; !bytes.Equal(got, []byte{1, 0, 2, 0}) {
		t.Fatalf("first audio = %v, want first connection audio", got)
	}
	...
	if got := len(dials); got != 2 {
		t.Fatalf("websocket dials = %d, want 2 after UpdateOptions", got)
	}
...
func TestElevenLabsTTSStreamSSMLParsingHoldsIncompleteReferenceTag(t *testing.T) {
	messages := make(chan map[string]any, 20)
	serverErr := make(chan error, 1)
	clientConn, serverConn := net.Pipe()
	go runElevenLabsTTSWebsocketServer(messages, serverConn, serverErr)
...
completeTag:
	...
	want := `<phoneme alphabet="ipa" ph="h eh l ow">hello</phoneme> `
	if tag["text"] != want || tag["context_id"] != contextID {
		t.Fatalf("completed SSML packet = %#v, want complete reference tag %q for context %q", tag, want, contextID)
	}
}
...
func TestElevenLabsTTSWebsocketMalformedJSONReturnsAPIStatusErrorLikeReference(t *testing.T) {
	serverErr := make(chan error, 1)
	clientConn, serverConn := net.Pipe()
	go runElevenLabsTTSMalformedJSONWebsocketServer(serverConn, serverErr)
	...
	if !strings.Contains(statusErr.Error(), "invalid character") {
		t.Fatalf("Next error = %v, want JSON parse detail", statusErr)
	}
	if err := stream.Close(); err != nil {
		t.Fatalf("Close() error = %v", err)
	}
...
func TestElevenLabsTTSWebsocketIgnoresBinaryMessageLikeReference(t *testing.T) {
	serverErr := make(chan error, 1)
	clientConn, serverConn := net.Pipe()
	go runElevenLabsTTSBinaryThenAudioWebsocketServer(serverConn, serverErr)
	...
	audio, err := stream.Next()
	if err != nil {
		t.Fatalf("Next error = %v, want audio after ignored binary frame", err)
	}
	if audio == nil || audio.Frame == nil || !bytes.Equal(audio.Frame.Data, []byte{1, 0, 2, 0}) {
		t.Fatalf("Next audio = %#v, want provider PCM frame after ignored binary message", audio)
	}
	if err := stream.Close(); err != nil {
		t.Fatalf("Close() error = %v", err)
	}
...
func runElevenLabsTTSSharedContextWebsocketServer(conn net.Conn, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
...
func runElevenLabsTTSOneContextWebsocketServer(conn net.Conn, audio []byte, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
...
func runElevenLabsTTSOneContextWebsocketServerNoDrain(conn net.Conn, audio []byte, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
...
func runElevenLabsTTSMalformedJSONWebsocketServer(conn net.Conn, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
	...
	if err := writeElevenLabsServerWebsocketTextFrame(conn, []byte("{not-json")); err != nil {
		errCh <- err
		return
	}
...
func runElevenLabsTTSBinaryThenAudioWebsocketServer(conn net.Conn, errCh chan<- error) {
	defer conn.Close()
	reader := bufio.NewReader(conn)
	req, err := http.ReadRequest(reader)
	if err != nil {
	...
	if err := writeElevenLabsServerWebsocketBinaryFrame(conn, []byte{0xff, 0x00, 0x7f}); err != nil {
		errCh <- err
		return
	}
	if err := writeElevenLabsServerWebsocketJSONFrame(conn, map[string]any{
...
func writeElevenLabsServerWebsocketJSONFrame(w io.Writer, msg map[string]any) error {
	...
	return writeElevenLabsServerWebsocketTextFrame(w, payload)
}

func writeElevenLabsServerWebsocketTextFrame(w io.Writer, payload []byte) error {
	return writeElevenLabsServerWebsocketFrame(w, 0x1, payload)
}

func writeElevenLabsServerWebsocketBinaryFrame(w io.Writer, payload []byte) error {
	return writeElevenLabsServerWebsocketFrame(w, 0x2, payload)
}

func writeElevenLabsServerWebsocketFrame(w io.Writer, opcode byte, payload []byte) error {
	if opcode == 0 {
		return errors.New("missing websocket opcode")
	}
	header := []byte{0x81}
