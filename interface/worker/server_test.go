...
func TestValidateRunPreconditionsKeepsTransportCredentialsSeparated(t *testing.T) {
	tests := []struct {
		name      string
		...
		{
			name: "agora does not require livekit credentials",
			options: WorkerOptions{
				Transport: WorkerTransportAgora,
			},
		},
		{
			name: "livekit still requires url",
			options: WorkerOptions{
				Transport: WorkerTransportLiveKit,
				APIKey:    "key",
				APISecret: "secret",
			},
			wantError: "ws_url is required",
		},
		{
			name: "livekit still requires api key",
			options: WorkerOptions{
				Transport: WorkerTransportLiveKit,
				WSRL:      "wss://livekit.example",
				APISecret: "secret",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err == nil || !strings.Contains(err.Error(), tt.wantError) {
				t.Fatalf("validateRunPreconditions() error = %v, want %q", err, tt.wantError)
			}
		})
	}
}
...
func TestRunResetsRetryAfterSuccessfulWorkerConnection(t *testing.T) {
	stubWorkerHTTPListener(t)
	oldSleep := workerRetrySleep
	oldDial := workerDialContext
	t.Cleanup(func() {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		msg := &livekit.ServerMessage{
			Message: &livekit.ServerMessage_Register{
				Register: &livekit.RegisterWorkerResponse{WorkerId: "worker-retry-reset"},
			},
		}
		data, err := proto.Marshal(msg)
		if err != nil {
	...
	for i := 0; i < 3; i++ {
		...
		case <-registered:
		case err := <-doneCh:
			t.Fatalf("Run() returned after %d successful registrations: %v", i, err)
		case <-time.After(time.Second):
			t.Fatalf("Run() registered %d times, want retry counter reset through third registration", i)
		}
	}
	cancel()
	select {
