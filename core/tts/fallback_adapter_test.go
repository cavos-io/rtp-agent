...
func TestFallbackAdapterEmitsAvailabilityChanges(t *testing.T) {
	...
	if available.TTS != primary {
		t.Fatalf("available TTS = %v, want primary", available.TTS.Label())
	}
...
func TestFallbackChunkedStreamMarksProviderUnavailableAfterAudioError(t *testing.T) {
	...
	primary := &metadataTTS{
		...
		numChannels: 1,
		chunked: &metadataChunkedStream{events: []*SynthesizedAudio{
			{Frame: &model.AudioFrame{Data: []byte("primary-a")}},
			{Frame: &model.AudioFrame{Data: []byte("primary-b")}},
		}, err: providerErr},
	}
	fallback := &metadataTTS{
		label:       "fallback",
	...
	adapter := NewFallbackAdapter([]TTS{primary, fallback})
	changes := make(chan AvailabilityChangedEvent, 1)
	adapter.OnAvailabilityChanged(func(event AvailabilityChangedEvent) {
		changes <- event
	})
	...
	if got := string(audio.Frame.Data); got != "primary-a" {
		t.Fatalf("first audio data = %q, want primary-a", got)
	}
	audio, err = stream.Next()
	if err != nil {
		t.Fatalf("second Next returned error: %v", err)
	}
	if got := string(audio.Frame.Data); got != "primary-b" {
		t.Fatalf("second audio data = %q, want primary-b", got)
	}
	if !audio.IsFinal {
	...
	audio, err = stream.Next()
	if audio != nil {
		t.Fatalf("third Next audio = %#v, want nil", audio)
	}
	if !errors.Is(err, io.EOF) {
		t.Fatalf("third Next error = %v, want io.EOF", err)
	}
	if fallback.synthesizeCalls != 0 {
		t.Fatal("fallback TTS was called after primary emitted audio")
	}

	unavailable := receiveTTSAvailabilityChange(t, changes)
	if unavailable.TTS != primary {
		t.Fatalf("availability change TTS = %p, want primary %p", unavailable.TTS, primary)
	}
	if unavailable.Available {
		t.Fatal("availability change Available = true, want false")
	}
}
...
func TestFallbackSynthesizeStreamReportsNoAudioForAPIStatus499BeforeAudio(t *testing.T) {
	fallback := &metadataTTS{
		label:        "fallback",
		sampleRate:   24000,
		numChannels:  1,
...
func TestFallbackSynthesizeStreamMarksProviderUnavailableAfterAudioError(t *testing.T) {
	providerErr := errors.New("provider failed after audio")
	primary := &metadataTTS{
		label:        "primary",
		sampleRate:   24000,
...
func TestFallbackChunkedStreamFallsBackWhenNormalizationFailsBeforeAudio(t *testing.T) {
	primary := &metadataTTS{
		label:       "primary",
		sampleRate:  16000,
		numChannels: 1,
...
func TestFallbackSynthesizeStreamFallsBackWhenNormalizationFailsBeforeAudio(t *testing.T) {
	primary := &metadataTTS{
		label:        "primary",
		sampleRate:   16000,
		numChannels:  1,
...
func TestFallbackSynthesizeStreamIgnoresEmptyText(t *testing.T) {
	...
	if len(buffer) != 0 {
		t.Fatalf("input buffer = %#v, want no buffered empty text", buffer)
	}
	if err := stream.PushText("hello"); err != nil {
...
func TestFallbackSynthesizeStreamDoesNotPushEmptyTextToActiveProvider(t *testing.T) {
	providerStream := &metadataSynthesizeStream{}
	adapter := NewFallbackAdapter([]TTS{
		&metadataTTS{
			label:        "primary",
	...
	if len(providerStream.calls) != 0 {
		t.Fatalf("provider calls = %#v, want no empty PushText forwarded", providerStream.calls)
	}
}
...
func TestFallbackSynthesizeStreamDoesNotFallbackAfterAudio(t *testing.T) {
	...
	if adapter.status[0].available {
		t.Fatal("first availability = true after post-output stream error, want false")
	}
	select {
	case got := <-errCh:
...
func TestFallbackSynthesizeStreamEndsInputOnRetryAfterEndInput(t *testing.T) {
	...
	if strings.Join(recovered.calls, ",") != strings.Join(wantCalls, ",") {
		t.Fatalf("replayed stream calls = %#v, want EndInput without extra flush: %#v", recovered.calls, wantCalls)
	}
	if strings.Contains(strings.Join(recovered.calls, ","), "flush") {
		t.Fatalf("replayed stream calls = %#v, want no flush generated by EndInput", recovered.calls)
	}
}
...
func receiveTTSAvailabilityChange(t *testing.T, changes <-chan AvailabilityChangedEvent) AvailabilityChangedEvent {
	t.Helper()
	select {
	case event := <-changes:
		return event
