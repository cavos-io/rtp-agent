...
type hyphenationPattern struct {
	points []int
	next   map[rune]*hyphenationPattern
}

var (
	hyphenatorOnce      sync.Once
	referenceHyphenator *hyphenationPattern
	referenceExceptions map[string][]int
)

func getReferenceHyphenator() (*hyphenationPattern, map[string][]int) {
	hyphenatorOnce.Do(func() {
		referenceHyphenator = buildHyphenationPatterns(referenceHyphenationPatterns)
		referenceExceptions = buildHyphenationExceptions(referenceHyphenationExceptions)
	})
	return referenceHyphenator, referenceExceptions
}

func buildHyphenationPatterns(patterns string) *hyphenationPattern {
	root := &hyphenationPattern{next: make(map[rune]*hyphenationPattern)}
	for _, pattern := range strings.Fields(patterns) {
		insertHyphenationPattern(root, pattern)
	}
	return root
}

func insertHyphenationPattern(root *hyphenationPattern, pattern string) {
	chars := make([]rune, 0, len(pattern))
	points := []int{0}
	for _, char := range pattern {
		if char >= '0' && char <= '9' {
			points[len(points)-1] = int(char - '0')
			continue
		}
	...
	for _, char := range chars {
		if node.next == nil {
			node.next = make(map[rune]*hyphenationPattern)
		}
		next := node.next[char]
		if next == nil {
			next = &hyphenationPattern{next: make(map[rune]*hyphenationPattern)}
			node.next[char] = next
		}
		node = next
	}
...
func buildHyphenationExceptions(exceptions string) map[string][]int {
	result := make(map[string][]int)
	for _, exception := range strings.Fields(exceptions) {
		var word strings.Builder
		breaks := make([]int, 0, strings.Count(exception, "-"))
		for _, char := range exception {
			if char == '-' {
				breaks = append(breaks, word.Len())
				continue
			}
			word.WriteRune(char)
		}
		result[word.String()] = breaks
	}
	return result
}

// HyphenateWord returns English hyphenation pieces compatible with the
// LiveKit basic tokenizer reference.
func HyphenateWord(word string) []string {
	if len(word) <= 4 {
		return []string{word}
	}
	root, exceptions := getReferenceHyphenator()
	if breaks, ok := exceptions[strings.ToLower(word)]; ok {
		return splitWordAtBreaks(word, breaks)
	}
	return hyphenateWordWithPatterns(root, word)
}

func hyphenateWordWithPatterns(root *hyphenationPattern, word string) []string {
	wordRunes := []rune(word)
	work := []rune("." + strings.ToLower(word) + ".")
	points := make([]int, len(work)+1)

	for i := range work {
		node := root
		for _, char := range work[i:] {
			next := node.next[char]
			if next == nil {
				break
			}
			...
			for j, point := range node.points {
				if i+j < len(points) && point > points[i+j] {
					points[i+j] = point
				}
			}
		}
	...
	for i, char := range wordRunes {
		pieces[len(pieces)-1] += string(char)
		if i+2 < len(points) && points[i+2]%2 == 1 {
...
func splitWordAtBreaks(word string, breaks []int) []string {
...
const referenceHyphenationPatterns = `    .ach4 .ad4der .af1t .al3t .am5at .an5c .ang4 .ani5m .ant4 .an3te .anti5s
...
const referenceHyphenationExceptions = `as-so-ciate as-so-ciates dec-li-na-tion oblig-a-tory phil-an-thropic present
