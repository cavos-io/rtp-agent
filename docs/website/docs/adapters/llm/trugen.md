...
# TruGen LLM Adapter for RTP Agent

TruGen provides specialized models for generating highly accurate and factual responses in technical or regulated industries.
...
Below is a basic conceptual example demonstrating how to initialize the TruGen LLM adapter within an RTP Agent session:
...
func main() {
	...
	// Initialize the TruGen LLM adapter
	llmProvider, err := trugen.NewProvider(
